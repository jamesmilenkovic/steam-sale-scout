...
/** Create the catalog table if it doesn't exist yet. Coder's call (per
 * SPEC.md, "CREATE TABLE IF NOT EXISTS on the sync path — Coder's call,
 * local-only app") over `wrangler d1 migrations`: this is a single-table,
 * single-environment, local-only app — a migrations directory would be
 * pure ceremony here. Safe to call on every request (idempotent, cheap). */
...
export async function getCatalogStats(env) {
  ...
  return { total, matched, unmatched, pending: total - matched };
}
...
export async function selectMatchedCatalogRows(env) {
  const result = await env.FPM_DB.prepare(
    `SELECT appid, name, owners, positive, negative, wilson, main_hours, match_method
     FROM fpm_catalog WHERE main_hours IS NOT NULL`,
  ).all();
  return result.results || [];
}
...
export async function resolveHltbBatch(env, ctx, items) {
  ...
  for (const item of toResolve) {
    let found = false;
    for (let attempt = 0; attempt < hltbPollMaxAttempts; attempt++) {
      const { cached, data } = await getCachedHltb(env, item.appid);
      if (cached) {
        await writeHltbResultToD1(env, item.appid, data, Date.now());
        found = true;
 ...
 * union owned/deal floor-passers (informational source flags, used only to
 * prioritize the HLTB batch below) -> select up to FPM_SYNC_BATCH NULL-
 * main_hours rows in priority order -> resolve them against HLTB, writing
 * through to D1. Never throws — a failure in any one step just means this
 * run made less progress than hoped; the next sync run picks up from
 * whatever's already in D1 (resumable by construction).
 * @param {object} env
 * @param {object} ctx
...
export async function runFpmSyncPipeline(env, ctx, deps) {
  ...
  await resolveHltbBatch(env, ctx, batch);
}
